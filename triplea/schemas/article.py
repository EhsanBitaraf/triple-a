...
class Keyword(BaseModel):
    ...
    IS_Major: Optional[bool] = Field(
        description="The MajorTopic attribute is set to True (for Yes) when the MeSH Heading/Keyword alone is a central concept of the article"
    )
...
class Author(BaseModel):
    ...
    LastName: Optional[str] = Field(
        description="contains the surname or the single name used by an individual, even if that single name is not considered to be a surname"
    )
