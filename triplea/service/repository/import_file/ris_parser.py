...
def _parse_ris_block(lines, sourcebanktype=SourceBankType.UNKNOWN):
    ...
    for l in range(0, len(lines)):
        ...
        if e == "TY":  # Type of reference. Must be the first tag.
            ...
            e == "TI"
        ):  # (Primary) title, e.g. title of entry/grant/podcast/work, case name, or name of act.
            a.Title = v
            # print(f"{e} -> {v}")
        elif (
            e == "T2"
        ):  # Secondary title, journal, periodical, publication title, code, title of weblog, series title, book title, image source program, conference name, dictionary title, periodical title, encyclopedia title, committee, program, title number, magazine, collection title, album title, newspaper, published
            a.Journal = v
            # print(f"{e} -> {v}")
            # All
            ...
            e == "J2"
        ):  # Alternate title, e.g. alternate journal, abbreviated publication, abbreviation, or alternate magazine. If possible, it should be a standard abbreviation, preferably using the Index Medicus style including periods. This field is used for the abbreviated title of a book or journal name, the latter mapped to T2.
            pass
            # print(f"{e} -> {v}")
            ...
            e == "AB"
        ):  # Abstract or synopsis.[6][14][8][15][17][18][19] Notes. Synonym of N2.
            a.Abstract = v
            # print(f"{e} -> {v}")
        elif (
            e == "KW"
        ):  # Keyword/phrase. Must be at most 255 characters long. May be repeated any number of times to add multiple keywords
            if a.Keywords is None:
                a.Keywords = []
            a.Keywords.append(Keyword(Text=v))
        elif (
            e == "AU"
        ):  # (Primary) author/editor/translator, e.g. author, artist, created by, attribution, programmer, investigators, editor, director, interviewee, cartographer, composer, reporter, inventor, or institution. The tag must be repeated for each person. Synonym of A1.
            ...
            e == "A2"
        ):  # Secondary author/editor/translator, e.g. editor, performers, sponsor, series editor, reporter, institution, name of file, producer, series director, department, interviewer, issuing organization, recipient, or narrator. The tag must be repeated for each person. Synonym of
            if a.Authors is None:
                a.Authors = []
            a.Authors.append(Author(FullName=v))
            # Not All
            ...
            e == "C2"
        ):  # Custom 2, e.g. PMCID, credits, year published, unit of observation, date cited, congress number, contact address, area, form of composition, issue, issue date, recieipients e-mail, or report number.[6][14][18][20]
            pass
            a.PMC = v
            # print(f"{e} -> {v}")
            # PMC10770784
            ...
            e == "DA"
        ):  # Date, e.g. date accessed, last update date, date decided, date of collection, date released, deadline, date of code edition, or date enacted.
            pass
            # print(f"{e} -> {v}") # Not All
            ...
            e == "ID"
        ):  # Reference identifier, may be limited to 20 alphanumeric characters.
            pass
            # print(f"{e} -> {v}")
            # ?
            ...
            e == "PB"
        ):  # Publisher, e.g. court, distributor, sponsoring agency, library/archive, assignee, institution, source, or university / degree grantor.
            pass
            # print(f"{e} -> {v}")
            ...
            e == "SP"
        ):  # Pages, description, code pages, number of pages, first/start page, or running time.
            pass
            # print(f"{e} -> {v}")
            # 261-270
            # e50865
            ...
            e == "T3"
        ):  # source, title of show, section title, academic department, or full journal name.
            pass
            # print(f"{e} -> {v}")
            # ?
        elif (
            e == "VL"
        ):  # Volume, code volume, access year, reporter volume, image size, edition, amount requested, rule number, volume/storage container, number, patent version number, code number, or degree.
            pass
            # print(f"{e} -> {v}")
            # 2016-December
            # 52
            ...
            e == "SE"
        ):  #  Section, screens, code section, message number, pages, chapter, filed date, number of pages, original release date, version, e-pub date, duration of grant, section number, start page, international patent number, or running time.
            pass
            # print(f"{e} -> {v}")
