...
def _convert_full_text2string_arxiv(article:Article):
    if SETTINGS.AAA_FULL_TEXT_REPO_TYPE =="Directory":
        filepath = os.path.join(SETTINGS.AAA_FULL_TEXT_DIRECTORY,
                                f"{article.ArxivID}.pdf")
        ...
        if SETTINGS.AAA_FULL_TEXT_STRING_REPO_TYPE=="Database":
            ...
            filepath = os.path.join(SETTINGS.AAA_FULL_TEXT_STRING_DIRECTORY,
                                    f"{article.ArxivID}.txt")   
            f = open(filepath, "w")
            f.write(fstr)
            f.close()         
            article.FullTextMetadata = {
