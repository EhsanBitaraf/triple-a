...
def convert_arepo_mongodb_to_tinydb(move_oreginal_article: Optional[bool] = False,
    ...
    for id in l_pmid:
        ...
        if destination_db.is_article_exist_by_pmid(pmid):
            ...
            n = n + 1
            try:
                if move_oreginal_article == False:
                    a['OreginalArticle'] = {}
                    a = Article(**a.copy())
                else:
                    a = Article(**a.copy())
                ...
                print()
                logger.ERROR(f'The copy of article {pmid} encountered an error.', deep = 3 )
