...
def export_pretrain_llm_in_dir(output_directory, Merge=True, proccess_bar=False, limit_sample=0,state = None):
    """
    ...
    if state is None:
        ...
        logger.INFO(f"{str(len(l_pmid))} Article(s) is in state {str(state)} in Article Repository")
