...
def configuration(command):
    if command == "info":
        ...
        if os.path.isfile(ENV_PATH_FILE):
            logger.INFO(f"Env file is exist.")
        else:
            logger.INFO(f"Env file is not exist.")
        ...
        if TRIPLEA_DB_TYPE == "MongoDB":
            AAA_MONGODB_CONNECTION_URL = click.prompt('AAA_MONGODB_CONNECTION_URL', default="mongodb://user:pass@127.0.0.1:27017/")
            AAA_MONGODB_DB_NAME = click.prompt('AAA_MONGODB_DB_NAME', default="articledata")
        elif TRIPLEA_DB_TYPE == "TinyDB":
            AAA_TINYDB_FILENAME = click.prompt('AAA_TINYDB_FILENAME', default="articledata.json")
        ...
        AAA_REFF_CRAWLER_DEEP = click.prompt('AAA_REFF_CRAWLER_DEEP', default=1 ,type=int)
        AAA_CITED_CRAWLER_DEEP = click.prompt('AAA_CITED_CRAWLER_DEEP', default=1 ,type=int)

        logger.WARNING("The settings will change as follows: ")
        logger.WARNING(f"   TRIPLEA_DB_TYPE:{TRIPLEA_DB_TYPE} ")
        if TRIPLEA_DB_TYPE == "MongoDB":
            logger.WARNING(f"   AAA_MONGODB_CONNECTION_URL: {AAA_MONGODB_CONNECTION_URL} ")
        ...
        if click.confirm('Do you want to continue?'):
            with open (ENV_PATH_FILE , 'w') as file:  
                ...
                if TRIPLEA_DB_TYPE == "MongoDB":
                    file.writelines(f"AAA_MONGODB_CONNECTION_URL={AAA_MONGODB_CONNECTION_URL} \n")
